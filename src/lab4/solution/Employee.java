 ...
 * -- Code that is repeated (Don't Repeat Yourself or DRY) is encapsulated in
 *    a helper method called "getFormattedDate". Such methods don't have to be 
 *    private but this one is because it has no use elsewhere.
 * -- Methods that need to be called in a specific order are encapsulated in
 *    a single parent method called "doFirstTimeOrientation". This assures that
 *    the order will be followed. But even if order where not a priority this
 *    code would still benefit from the use of parent method/helper method
 *    architecture. If forces each method routine to be simple, for easy
 *    understanding and editing, and it makes the code more readable. Further,
 *    it means that the pubic methods are reusable by themselves.
 * -- Not all methods are public. Having too many public methods makes a class
 *    harder to understand and more complex. Some of the methods below are
 ...
 *    hires Employees. When an employee is hired the HrManager issues the
 *    command to the employee to do the orientation, but the HrManager is 
 *    acting on behalf of the Company so it is the company that is actually
 *    hiring the employee and mandating orientation. These tasks are all delegated
 *    to the hidden objects inside. Company delegates to HrManager, HrManager
 *    delegates to Employee -- just like the real world!
 *
